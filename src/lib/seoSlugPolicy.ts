...
export function getSeoSlugHubPolicy(keyword: string): SeoSlugHubPolicy {
  ...
  if (/รับเหมา|รับประมูล|รับซื้อทรัพย์สิน|รับซื้ออุปกรณ์ไอทีเก่า|รับซื้ออุปกรณ์ไอที$/i.test(keyword)) {
    return { action: 'KEEP', primary: null, reason: 'B2B/auction intent without money page', score: 78, cluster: 'b2b' };
  }
  if (
    ...
    return { action: 'KEEP', primary: null, reason: 'infra/network intent without money page', score: 76, cluster: 'infra' };
  }
  if (/^รับซื้อ\s*ups$/i.test(keyword.trim()) || /แบตเตอรี่\s*ups|ups\s*server|ups\s*rack/i.test(keyword)) {
    return { action: 'KEEP', primary: null, reason: 'UPS category hub', score: 74, cluster: 'ups' };
  }
  if (
